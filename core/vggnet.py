...
vgg_layers = ['conv1_1', 'relu1_1', 'conv1_2', 'relu1_2', 'pool1',
              ...
              'conv5_1', 'relu5_1', 'conv5_2', 'relu5_2', 'conv5_3', 'relu5_3', 'conv5_4', 'relu5_4','pool5','fc6','relu6','fc7','relu7']
...
class Vgg19(object):
    ...
    def _fc6(self,x,w,b):
        ...
        x_reshape=tf.reshape(x,[-1, 25088])
        w_reshape= tf.reshape(w,[25088,4096])
        
       	return tf.nn.bias_add(tf.matmul(tf.cast(x_reshape,tf.float32),tf.cast(w_reshape,tf.float32)),b)

    def _fc7(self,x,w,b):
          
        w_reshape = tf.reshape(w,[4096,4096]) 
        return tf.nn.bias_add(tf.matmul(tf.cast(x,tf.float32),tf.cast(w_reshape,tf.float32)),b)

    def build_model(self):
        for i, layer in enumerate(vgg_layers):
           
            layer_type = layer[:4]
            if layer_type == 'conv' and layer !='fc6' and layer !='fc7':
                if layer == 'conv1_1':
                    h = self.images
            ...
            if layer == 'fc6':
                ...
                h=self._fc6(h,self.params[layer]['w'],self.params[layer]['b'])
               
               
            if layer == 'fc7':
                h=self._fc7(h,self.params[layer]['w'],self.params[layer]['b'])
                self.features = tf.reshape(h, [-1, 4096])
